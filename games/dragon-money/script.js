...
const LS_KEY = 'dragon_money_state_v11';

const state = {
  balance: 1000,
  betOptions: [0.25, 0.5, 1, 2, 5, 10, 20],
  betIndex: 2,
  freeSpins: 0,
  bonusMultiplier: 1,
  lastWin: 0,
...
function setBonusCharge(v){ const prev = state.bonusCharge; state.bonusCharge=Math.max(0,Math.min(100,Math.round(v))); if(el.bonusChargeLabel)el.bonusChargeLabel.textContent=`${state.bonusCharge}%`; if(el.bonusChargeFill)el.bonusChargeFill.style.width=`${state.bonusCharge}%`; if(el.lynxStage)el.lynxStage.classList.toggle('charged',state.bonusCharge>=100); if(prev < 100 && state.bonusCharge >= 100){ updateStatus('O Dragão está pronto para lançar o bônus!', 'bonus'); addHistory('O <b>Bônus do Dragão</b> ficou pronto para ser lançado.'); } }
function addBonusCharge(v){ setBonusCharge(state.bonusCharge+v); }

const reelProfiles = [
  { wild:1, scatter:1, gold:4, jade:4, envelope:5, firecrackers:5, accessory:5, a:10, k:10, q:10, j:10, ten:11 },
  { wild:2, scatter:1, gold:4, jade:4, envelope:5, firecrackers:5, accessory:5, a:9, k:10, q:10, j:10, ten:11 },
  { wild:1, scatter:1, gold:4, jade:4, envelope:5, firecrackers:5, accessory:5, a:10, k:10, q:10, j:10, ten:11 }
];
...
function generateGrid(){ return Array.from({length:9},(_,idx)=>weightedRandomSymbol(idx%3)); }
...
function addHistory(message){ const item=document.createElement('div'); item.className='history-item'; item.innerHTML=`<b>${new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</b> — ${message}`; el.history.prepend(item); while(el.history.children.length>12)el.history.removeChild(el.history.lastChild); }
...
async function playLynxBonusPot(baseWin){
  ...
  updateStatus('Dragão segurando a esfera... lançando bônus!', 'bonus');
  sfxLynx();
  ...
  addHistory(`O <b>Bônus do Dragão</b> foi liberado: bônus extra de <b>${money.format(bonus)}</b> (10x do ganho base).`);
  updateStatus(`BÔNUS DO DRAGÃO! +${money.format(bonus)} em bônus x10!`,'bonus');
...
async function spin(){
  ...
  if(outcome.wilds>0){ addBonusCharge(outcome.wilds * 34); addHistory(`WILD x${outcome.wilds}: o Bônus do Dragão acumulou energia.`); }
...
loadState(); state.turbo = false; makeInitialGrid(); renderGrid(); renderPaytable(); setupEvents(); updateUI(); refreshPotTarget(); addHistory('Fortune Lynx iniciado. O Bônus do Dragão agora só carrega quando aparece WILD.'); updateStatus('Pronto para girar.'); bootLoading();
